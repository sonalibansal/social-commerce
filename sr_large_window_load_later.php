				...
				if($flag==1)
				{
						 ...
						 { $handle = popen('C:\Python27\trim_final.py 2>&1', 'r');$read = fread($handle, 2096); pclose($handle); $newtext=$text.$read; 
							...
							if (strlen($newtext)>=1000)
									{	$x=(strlen($newtext)-1000);
										$x=$x+5;
										$rest = substr($newtext, 0,-$x);
										$rest=$rest."..";
										echo wordwrap($rest, 500, "\n", true);
										//echo "#";
									}
							else 
								
							{	
								echo wordwrap($newtext,500, "\n", true);
							    //echo "#";
							}
						}
